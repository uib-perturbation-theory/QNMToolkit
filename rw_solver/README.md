...
Numerical time-domain solver for the **Regge--Wheeler equation**, describing perturbations of a Schwarzschild black hole and their associated quasi-normal-mode ringdown.

The code evolves a scalar or gravitational perturbation in tortoise coordinates using:

- 4th-order Runge--Kutta time integration,
- 4th-order finite differences in space,
- a smooth windowed Regge--Wheeler potential,
- 4th-order Lagrange interpolation for observer extraction.
...
r
+
2M\ln\left(\frac{r}{2M}-1\right).
```

It maps the exterior Schwarzschild region $r\in(2M,\infty)$ to $r_*\in(-\infty,+\infty)$.

### Inverting the tortoise coordinate

Given $r_*$, the Schwarzschild radius $r$ can be recovered using the principal branch of the Lambert W function:
...
This inversion is implemented in `r_of_s(s, M)`. The code uses `scipy.special.lambertw` when available, and falls back to `mpmath.lambertw` otherwise.
...
To suppress spurious reflections from the grid boundaries, the potential is multiplied by a smooth window function:
...
The spatial operator is the second derivative minus the potential term:
...
One-sided stencils of equivalent order are applied at the boundary points.

### Time integration

The wave equation is written as a first-order system by defining

```math
\Pi \equiv \partial_t\Psi .
```

The evolution system is then

```math
\partial_t \Psi = \Pi,
\qquad
\partial_t \Pi = \mathcal{L}[\Psi].
```

The system is advanced in time using the standard 4th-order Runge--Kutta method:
...
- $A$: amplitude,
- $s_0$: pulse centre in tortoise coordinates,
...
If the README renderer does not support `substack`, the same expression can be read as: multiply over all $k=0,1,2,3$ except $k=j$.
